...
- **Cross-platform native installers.** Windows (MSI / NSIS), macOS (DMG, Apple Silicon and Intel), and Linux (AppImage, deb, rpm).
...
API keys are stored in the local SQLite settings table only. Nothing is uploaded to a SecBuddy-hosted service — there isn't one.
...
│   ├── components/       # Chat, Sidebar, ToolCard, FindingCard, …
│   ├── routes/           # Home, Chat, Settings
│   ├── stores/           # Zustand stores (chat, settings, …)
│   └── lib/tauri.ts      # Typed wrappers around invoke() commands
├── src-tauri/            # Rust + Tauri 2 backend
│   ├── src/
│   │   ├── lib.rs        # Tauri commands, app setup
│   │   ├── rig_orchestrator/  # Agent loop powered by rig-core
│   │   ├── llm_client.rs # Provider config + connection tests
│   │   ├── mcp_client.rs # MCP server runtime
│   │   ├── tool_registry.rs / tool_runner.rs  # Local CLI execution
│   │   ├── governance.rs # Approval policy
│   │   └── db/           # SQLite schema & queries
│   ├── tools.json        # Built-in tool catalog
│   └── tauri.conf.json   # Tauri bundler/window config
...
3. Tag and push:
   ...
   ```
4. The `Release` workflow will build Windows / macOS (Apple Silicon + Intel) / Linux installers and create a **draft GitHub Release** with all assets attached.
